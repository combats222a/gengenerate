...
export function FavoritesList({ items }: FavoritesListProps) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {favorites.map((item) => (
        // cv-auto — та же виртуализация через content-visibility, что и
        // в каталоге (Этап 11); здесь это чисто клиентский список, но
        // класс одинаковый, чтобы не плодить два подхода на одно и то же.
        <div key={item.id} className="cv-auto">
          <GeneratorCard generator={{ ...item, icon: GENERATOR_ICON_BY_SLUG[item.slug] ?? Star }} />
        </div>
      ))}
    </div>
  );
